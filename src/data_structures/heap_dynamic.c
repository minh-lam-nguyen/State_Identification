...
struct s_heap{
    int** h; // Talbeau contenant les éléments
...
int doubleMaxHeapSize(Heap h){
    int** h2 = (int**)realloc(h->h, sizeof(int*) * 2 * h->maxSize);
    if(h2 == NULL){
        free(h);
        return 0;
    }
